...
public final class ProductionResolver {
  public record Production(UUID playerId, UUID settlementId, ResourceType resource, int amount) {}
    ...
    for (PlayerState p : game.players) {
      for (SettlementState s : p.settlements) {
        ...
        if (h != null
          ...
          report.add(new Production(p.id, s.id(), h.resource(), amount));
        }
      }
      if (p.hero.heroClass() == HeroClass.MERCHANT) {
        p.settlements.stream()
            ...
            .ifPresent(
                s -> {
                  ...
                  report.add(new Production(p.id, s.id(), h.resource(), 1));
                });
      }
    }
